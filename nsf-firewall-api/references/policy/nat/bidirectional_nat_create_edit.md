...
| dst_port | 是 | string | 入接口（三层主接口/三层子接口/vlan口/汇聚三层主接口/GRE接口，需在源安全区内） |
...
| interface | 是 | string | 出接口，最多接口数64个，参数为对象名称 |
...
| s_address_translation | 是 | int | 源地址转换类型：1-接口地址/2-网络对象 |
...
curl -X POST "https://{{host}}/north/nf/strategy/nat/nat_action/" \
  ...
  -d '{
    ...
    "nat_object": "",
    "port_map_type": "2",
    "internal_address": "ip123",
    "external_address": "ip123",
    "internal_port": "",
    "external_port": "",
    "health_check": "",
    "load_balance": "random",
